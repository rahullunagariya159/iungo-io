...
$query ="SELECT id,aboutyou FROM accounts Where spaceId='".$spaceId."' AND roomId = '".$roomId."' HAVING id != '".$_SESSION['id']."' ";
...
if($result = mysqli_fetch_row($row))
{
	...
	if($check_stats_row)
	{
		...
		$update_stats_query = "UPDATE stats SET chattime = chattime + 3,venueId='$category_id' WHERE roomId = '$roomId' AND participants = '$participants' LIMIT 1";
	...
	else
	{
		...
		$stats_query = "INSERT INTO `stats`(`participants`, `chattime`, `room`,`roomId`,`user1`,`user2`,`venueId`) VALUES ('$participants','$chattime','$room','$roomId','$user1','$user2','$category_id')";
