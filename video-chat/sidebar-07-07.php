		...
		$orignalFileNameVenue = basename($_FILES["venuePicture"]["name"]);
			...
			if($orignalFileNameVenue == "")
			{
				...
				if(empty($moderators) || $venueTypeCap != "Private")
				{
					...
					if($venueAction != "update")
					{
						$moderatorsVenueMem = "INSERT INTO venuemembers (UserID,VenueID,DateJoined,Role) VALUES ('$userId','$last_id','$current_date_time','member')";
				...
				{	
					...
					if($venueAction == "update"){
						...
						foreach ($insertNewMemValue as $key => $value) {
							...
							$newVenueModrator = "INSERT INTO venuemembers (UserID,VenueID,DateJoined,Role) VALUES ('$value','$venueID','$current_date_time','member')";
					...
					else
					{
						...
						$moderatorsVenueModrator = "INSERT INTO venuemembers (UserID,VenueID,DateJoined,Role) VALUES ('$value','$last_id','$current_date_time','member')";
...
<div class="sidebar_mn">
    ...
    if(isset($_SESSION['id'])){ ?>
    <button type="button" class="btn create-room  btn-create mt-3" data-toggle="modal" data-target="#createNewRoom">Create Room</button>
<?php } 
	
	if($findOwnerOfSpace > 0)
	{ ?>
		<button type="button" class="btn btn-create app-space-mem mt-3" >Approve your space member</button>
	<?php }
...
$query1 = "SELECT VenueID FROM venuemembers where UserID = '".$_SESSION['id']."' AND (Role != 'left' AND Role != 'pending') ";
